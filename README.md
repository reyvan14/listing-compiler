...
刷新浏览器不会丢掉工作。项目按版本化 schema 自动保存在**这台浏览器的本地存储**里，
界面上始终显示保存状态：`未保存` / `保存中` / `已保存` / `保存失败` / `已从备份恢复`。

**两类状态，分工明确。** 修订、校验、审批、证据、事实、图片检查、发布护照都由服务端
按作用域持久化，本来就扛得住刷新；把它们再抄一份进浏览器只会制造第二个「真相源」。
所以本地快照只存**画布本身**（tldraw 的 shapes / bindings / 节点内容 / 位置 / 视角）
加上解析服务端记录所需的**作用域与引用 ID**。

**恢复的是图，不是生成过程。** 刷新时先加载快照再决定是否播种 SKU 节点——重新跑一遍
生成代码会为已经做完的工作凭空造出新节点、新修订和新审计事件。E2E 断言刷新后
节点 ID、类型、坐标与连线数量完全一致，且修订历史仍然只有一条。

**损坏的新数据不会毁掉能用的旧数据。** 写入分三步：先写暂存键，再把上一份好的快照
提升为备份，最后换入当前键。当前存档解析失败时自动回落到备份并明确告知「已从备份恢复」；
两份都读不出来时报「无法解析」并**停止自动保存**，绝不用空白画布覆盖掉可能还能抢救的数据。
来自**更新版本**的存档会被拒绝而不是降级读取——强行读会静默丢掉新版本新增的内容。

**导入是替换，不是合并。** 导入前先校验 schema、体积、版本、引用 ID（重复 / 空值）
与凭证字段，然后**预览**将要导入的节点数、连线数与节点类型构成，确认后才替换当前画布。
本工具不支持合并两个项目，界面上直说这一点。项目文件里出现任何疑似凭证字段
（`api_key` / `authorization` / `cookie` / `private_key` / …）会**整体拒绝导入**
而不是悄悄剥掉——悄悄剥掉等于告诉对方「导出密钥没关系」。

操作：立即保存 / 导出项目 / 导入项目 / 恢复上一份备份 / 新建项目 / 清除本地存档
（后两个都要二次确认）。体积超过浏览器配额时保存会**明确失败**并提示改用导出为文件；
过大的生成图 data URL 不进本地存储，界面会说明有几张没存进去，导出项目则完整保留。

服务端的证据与运行时数据（`api/evidence_store/`）已在 `.gitignore` 中，
不随代码版本库分发。

## Phase 2：多模态读取 → 政策监视 → 域操作 → 反馈 → 分镜

### 多模态读取与事实注册表（2A / 2B）

上传的图片与文档现在会被**读取**，但读取从不等于相信。

- **看图不能确立宣称。** 认证、安全、材质、数值、性能这几类宣称无法靠外观确立，
  图片来源的候选事实一律拒绝这些类别，并把拒绝原因显示出来。用 OCR 读出包装上印的
  数字是允许的——那是在读文字，不是在看长相——读出来仍然是「待确认」。
- **提取从不产生已核实事实。** 每条候选带取值、置信度、来源文件、页码或边界框、
  提取方式与审核状态；确认读数只表示「机器读对了」，事实进账本时仍是待确认，
  没有对应文档时甚至是「无证据」——发布闸门会拦住它。
- **上传文本是数据，不是指令。** 只有已核实的**结构化事实**会进入生成提示词，
  原始 OCR 文本与文档摘录永远不会。
- **OCR 是可选的，缺席时如实说。** `api/ocr.py` 定义 provider 接口并实现了真实的
  Tesseract 适配器（CLI + 固定参数数组，无 shell；词级边界框与置信度取自 TSV）。
  未安装引擎时返回 `manual_review` / `ocr_unavailable`，应用照常启动，
  不会有任何地方把它当成「检查通过」。界面把识别框画在原图上，允许人工更正后再确认——
  本项目自己的样例里 Tesseract 就把 `350 ml` 读成了 `350 mt`（置信度 63%）。
- **能力是声明的，不是假设的。** `supports_vision` / `supports_reference_image` /
  `supports_ocr` 来自各协议**有文档依据**的请求格式；legacy 的 `/images/generations`
  没有参考图字段，就不声称支持，也不会被塞进一个编造的参数。
- 三张写死的正则表变成**可扩展的事实注册表**：键、标签、宣称类型、数据类型、单位族与别名、
  抽取模式、归一化、冲突比较、是否需要证据。七个既有属性全部保留，新增十三个通用属性。

### 政策来源监视（2C）与市场边界（2G）

见上文「政策来源监视」小节。要点：白名单 + DNS 后校验 + 逐跳重定向校验，
只产生候选记录，规则仍需人工提交；没有自有快照的市场显示「政策未覆盖，需人工复核」，
绝不套用其他市场的规则。

### Agent 域操作（2D）

模型只能从**封闭清单**里挑一个操作名和类型化参数，永远不能指定接口、URL、方法、
路径或命令。参数用**正向白名单**校验（这一条是被测试逼出来的：最初的黑名单放过了
`rev-1; rm -rf /`）。只读操作在计划批准后即可执行；批准、导出、迁移、付费生成
需要**第二次确认**，确认令牌由该操作自身的参数派生，所以确认导出 A 不能授权导出 B。
每个请求带幂等键，重试返回首次结果而不是再做一次。发布不在清单上，且被显式列为禁止。

### 反馈实验室（2E）

导入表格 → 确定性统计 → 观测信号 → 候选修订 → 实验记录。
**相关性就说是相关性**：每次对比同时给出两组样本量与时间窗口，小样本标为可能是噪声，
因果免责声明写在数据里而不是脚注里。没有任何地方预测提升幅度。
信号只会变成**候选修订**（走既有审核流程，approved 的修订不会被覆盖），不会直接改文案。
单行解析失败不影响其余行导入。

### 可核验分镜与内容包（2F）

分镜可读、可重排、可单独重跑。默认十五秒四段：钩子 / 演示 / 有证据支撑的卖点 / 收尾。
生成前显示**精确的付费调用次数**，超过一次需显式确认；重跑单个失败分镜只花一次调用，
已成功的分镜不会重复付费。取消由 run token 强制执行——晚到的结果会被拒绝而不是覆盖。
进度是「分镜 2/4 已生成」这样可以对着分镜表核对的计数，没有百分比，也没有计时器。
**没有合成就不说合成**：本机没有 FFmpeg，内容包里是各分镜片段 + WebVTT/SRT 字幕 +
旁白文本，并明确说明未合成成片；合成适配器仍按固定参数数组、输入校验、时长上限与
输出探测的规范写好。未配置 TTS 时导出旁白文本，不假装生成了语音。

### 可选重型依赖对部署的影响

| 依赖 | 缺席时的行为 | 启用方式 | 影响 |
|---|---|---|---|
| Tesseract OCR | 图片文字不自动读取，相关事实一律 `manual_review`；应用正常启动 | `apt install tesseract-ocr tesseract-ocr-chi-sim`，或用 `LISTING_OCR_CMD` 指向二进制 | 约 +40 MB 磁盘；单张图 CPU 占用数百毫秒到数秒，已设 20s 超时与 12 MB / 40 MP 上限 |
| FFmpeg | 不合成成片，内容包只含独立片段与字幕，并如实说明 | `apt install ffmpeg`，或用 `LISTING_FFMPEG_CMD` 指定 | 约 +100 MB 磁盘；合成为 CPU 密集，已设 120s 超时与 64 MB/片段上限 |
| TTS 适配器 | 导出旁白文本与字幕，不生成语音 | 配置 `LISTING_TTS_BASE_URL` 与 `LISTING_TTS_API_KEY` | 取决于所选服务；本仓库不内置任何 TTS 供应商 |

三者都是**可选**的：缺席只改变功能范围，不会导致启动失败，也不会让任何检查从
「未判定」变成「通过」。

### 真机冒烟脚本

`api/scripts/smoke_providers.py` 在配置了凭证时手动运行，各发一次文本 / 图片 / 视频请求。
它不打印任何密钥、提示词或响应正文，只报告协议、耗时与结果形状；
未配置凭证时跳过而不是失败，也不假设装了 OCR。它**不在**自动化测试里——
测试套件不应依赖付费外部服务。
