    ...
    <div ref={dockRef} className={styles.dock}>
      ...
      {open === 'add' && (
        <section className={styles.menu} aria-label="添加节点">
          <div className={styles.menuTitle}>创作节点</div>
          ...
          <div className={styles.menuTitle}>工位</div>
          <button
            type="button"
      ...
      {open === 'help' && (
        <section className={styles.menu} aria-label="功能说明">
          <div className={styles.menuTitle}>工位怎么用</div>
      ...
      {open === 'contact' && (
        <section className={styles.menu} aria-label="联系方式">
          ...
          <p className={styles.help}>天池初赛演示工位。评委路径走画布：填入演示 → 生成 → 三台检查。不登广告账户。</p>
        </section>
      )}
    </div>
  )
