/**
 * This file contains functions for working with ports and connections on nodes.
 */
import { createComputedCache, Editor, TLShapeId } from 'tldraw'
import { getConnectionBindings } from '../connection/ConnectionBindingUtil'
import { PortDataType } from '../constants'
import { PortId } from '../ports/Port'
import { NodeShape } from './NodeShapeUtil'
import { getNodeDefinition, getNodeOutputInfo, getNodeTypePorts } from './nodeTypes'
import {
	areAnyInputsOutOfDate,
	InfoValues,
	isMultiInfoValue,
	PipelineValue,
	STOP_EXECUTION,
} from './types/shared'

/**
 * Get the ports for a node. This is cached, because we only want to re-evaluate it when the
 * underlying records change.
 */
export function getNodePorts(editor: Editor, shape: NodeShape | TLShapeId) {
	return nodePortsCache.get(editor, typeof shape === 'string' ? shape : shape.id) ?? {}
}
const nodePortsCache = createComputedCache('ports', (editor: Editor, node: NodeShape) =>
	getNodeTypePorts(editor, node)
)

/**
 * A connection from one node to another.
 */
export interface NodePortConnection {
	connectedShapeId: TLShapeId
	connectionId: TLShapeId
	terminal: 'start' | 'end'
	ownPortId: PortId
	connectedPortId: PortId
	order: number
}

/**
 * Get the connections for a node. This is cached.
 */
export function getNodePortConnections(
	editor: Editor,
	shape: NodeShape | TLShapeId
): NodePortConnection[] {
	return nodePortConnectionsCache.get(editor, typeof shape === 'string' ? shape : shape.id) ?? []
}
const nodePortConnectionsCache = createComputedCache(
	'port connections',
	(editor: Editor, node: NodeShape) => {
		const bindings = editor.getBindingsToShape(node.id, 'connection')

		const connections: NodePortConnection[] = []
		for (const binding of bindings) {
			const oppositeTerminal = binding.props.terminal === 'start' ? 'end' : 'start'
			const oppositeBinding = getConnectionBindings(editor, binding.fromId)[oppositeTerminal]
			if (!oppositeBinding) continue

			connections.push({
				connectedShapeId: oppositeBinding.toId,
				connectionId: binding.fromId,
				terminal: binding.props.terminal,
				ownPortId: binding.props.portId,
				connectedPortId: oppositeBinding.props.portId,
				order: binding.props.order ?? 0,
			})
		}

		return connections
	},
	{
		areRecordsEqual: (a, b) => a.id === b.id,
	}
)

/**
 * Get the values of the input ports for a node. This is cached.
 */
export function getNodeInputPortValues(editor: Editor, shape: NodeShape | TLShapeId): InfoValues {
	return nodeInputPortValuesCache.get(editor, typeof shape === 'string' ? shape : shape.id) ?? {}
}
const nodeInputPortValuesCache = createComputedCache(
	'node input port values',
	(editor: Editor, node: NodeShape) => {
		const connections = getNodePortConnections(editor, node)
		const sorted = [...connections].sort((a, b) => a.order - b.order)
		const ports = getNodePorts(editor, node)

		const values: InfoValues = {}
		for (const connection of sorted) {
			if (!connection || connection.terminal !== 'end') continue

			const connectedShapeOutputs = getNodeOutputPortInfo(editor, connection.connectedShapeId)
			if (!connectedShapeOutputs) {
				continue
			}

			const output = connectedShapeOutputs[connection.connectedPortId]
			if (!output) continue
			const port = ports[connection.ownPortId]

			if (port?.multi) {
				// Output ports are always single-valued; extract the scalar value.
				const singleValue = isMultiInfoValue(output) ? output.value[0] : output.value
				const existing = values[connection.ownPortId]
				if (existing && isMultiInfoValue(existing)) {
					existing.value.push(singleValue)
					existing.isOutOfDate = existing.isOutOfDate || output.isOutOfDate
				} else {
					values[connection.ownPortId] = {
						value: [singleValue],
						isOutOfDate: output.isOutOfDate,
						dataType: output.dataType,
						multi: true,
					}
				}
			} else {
				values[connection.ownPortId] = output
			}
		}

		return values
	},
	{
		areRecordsEqual: (a, b) => a.props === b.props,
	}
)

/**
 * Get the values of the output ports for a node. This is cached.
 */
export function getNodeOutputPortInfo(editor: Editor, shape: NodeShape | TLShapeId): InfoValues {
	return nodeOutputPortInfoCache.get(editor, typeof shape === 'string' ? shape : shape.id) ?? {}
}

export interface ClassifiedPortInputs {
	/** 文本类上游（prompt/创意策略/文本生成等）的值，按连线顺序。 */
	texts: string[]
	/** 图片类上游（图片生成/参考素材出图等）的 URL，按连线顺序。 */
	images: string[]
	/** 视频类上游的 URL，按连线顺序。 */
	videos: string[]
}

const IMAGE_URL_RE = /\.(png|jpe?g|webp|gif|bmp|avif|svg)(\?|#|$)/i
const VIDEO_URL_RE = /\.(mp4|webm|mov|m4v|m3u8|ogv)(\?|#|$)/i
const looksLikeImageUrl = (v: string) => /^data:image\//i.test(v) || IMAGE_URL_RE.test(v)
const looksLikeVideoUrl = (v: string) => /^data:video\//i.test(v) || VIDEO_URL_RE.test(v)
// 输出 dataType 为 'any' 但实际产出图片的源（如参考素材）：按节点类型补判为图片。
const IMAGE_SOURCE_NODE_TYPES = new Set(['reference_material'])

/**
 * 把连入某个输入端口的上游值，按「源输出端口的 dataType」分流成文本/图片/视频三组。
 * 用于「单一通用输入口」按上游类型路由：图片→参考图/首帧，文本→提示词。
 * 逐条连线读源输出（值+dataType），正确处理 multi 口混合类型（getNodeInputPortValues 只保留首个 dataType）。
 * 对 dataType='any' 的源（如 reference_material 出图）再按「源节点类型 / URL 特征」补判图片/视频。
 */
export function classifyPortInputs(
	editor: Editor,
	shape: NodeShape | TLShapeId,
	portId: PortId
): ClassifiedPortInputs {
	const id = typeof shape === 'string' ? shape : shape.id
	const conns = getNodePortConnections(editor, id)
		.filter((c) => c.terminal === 'end' && c.ownPortId === portId)
		.sort((a, b) => a.order - b.order)
	const result: ClassifiedPortInputs = { texts: [], images: [], videos: [] }
	for (const c of conns) {
		const info = getNodeOutputPortInfo(editor, c.connectedShapeId)?.[c.connectedPortId]
		if (!info) continue
		const value = isMultiInfoValue(info) ? info.value[0] : info.value
		// 仅取非空字符串值；STOP_EXECUTION 是 symbol，已被 typeof !== 'string' 一并过滤。
		if (typeof value !== 'string' || !value) continue
		const srcShape = editor.getShape(c.connectedShapeId)
		const srcType =
			srcShape && editor.isShapeOfType(srcShape, 'node') ? srcShape.props.node.type : undefined
		const dt = info.dataType
		if (dt === 'image' || (srcType && IMAGE_SOURCE_NODE_TYPES.has(srcType)) || looksLikeImageUrl(value)) {
			result.images.push(value)
		} else if (dt === 'video' || looksLikeVideoUrl(value)) {
			result.videos.push(value)
		} else {
			result.texts.push(value)
		}
	}
	return result
}
function hasStopExecution(input: InfoValues[string]): boolean {
	if (isMultiInfoValue(input)) {
		return input.value.some((v) => v === STOP_EXECUTION)
	}
	return input.value === STOP_EXECUTION
}

const nodeOutputPortInfoCache = createComputedCache(
	'node output port info',
	(editor: Editor, node: NodeShape): InfoValues => {
		const inputs = getNodeInputPortValues(editor, node)
		// 默认：任一输入为 STOP 则短路本节点全部输出为 STOP。但允许节点按自身状态退出
		// （如 load_image 持有自身图时不应被上游 STOP 掐断）。
		const shortCircuits = getNodeDefinition(editor, node.props.node).shortCircuitsOnInputStop(
			node.props.node
		)
		if (shortCircuits && Object.values(inputs).some(hasStopExecution)) {
			const ports = getNodePorts(editor, node)
			return Object.fromEntries(
				Object.values(ports)
					.filter((port) => port.terminal === 'start')
					.map((port) => [
						port.id,
						{
							value: STOP_EXECUTION as PipelineValue | STOP_EXECUTION,
							isOutOfDate: areAnyInputsOutOfDate(inputs),
							dataType: port.dataType,
						},
					])
			)
		}

		return getNodeOutputInfo(editor, node, inputs as InfoValues)
	},
	{
		areRecordsEqual: (a, b) => a.props === b.props,
	}
)

/**
 * Traverse the graph from a starting node, in a given direction, returning all the nodes that are
 * connected to it (including the starting node).
 */
export function getAllConnectedNodes(
	editor: Editor,
	startingNode: TLShapeId | NodeShape,
	direction?: 'start' | 'end'
) {
	const toVisit = [typeof startingNode === 'string' ? startingNode : startingNode.id]
	const found = new Set<TLShapeId>()

	while (toVisit.length > 0) {
		const nodeId = toVisit.shift()
		if (!nodeId) continue

		const node = editor.getShape(nodeId)
		if (!node || !editor.isShapeOfType(node, 'node')) continue

		if (found.has(node.id)) continue
		found.add(node.id)

		for (const connection of getNodePortConnections(editor, node)) {
			if (direction && connection.terminal !== direction) continue
			toVisit.push(connection.connectedShapeId)
		}
	}

	return found
}

/**
 * Get the data type of a port on a node. Returns the port's dataType.
 */
export function getPortDataType(
	editor: Editor,
	shapeId: TLShapeId,
	portId: PortId
): PortDataType | null {
	const ports = getNodePorts(editor, shapeId)
	return ports[portId]?.dataType ?? null
}
